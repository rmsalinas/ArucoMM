...
public:
    ...
    FrameExtractor();
     void setParams(std::shared_ptr<Feature2DSerializable>  feature_detector, const ucoslam::Params &params);
    ...
    std::vector<float> GetScaleFactors( ){  return _fdetector->GetScaleFactors();}
    std::vector<float> GetInvScaleFactors( ){auto sfc=_fdetector->GetScaleFactors();
                                             for(auto &v:sfc)v=1./v;return sfc; }
private:
